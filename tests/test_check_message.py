...
"""This module contains unit tests for the `can_form_message` function.
It tests various scenarios including basic cases, case insensitivity,
ignoring spaces, and cases with special characters and numbers."""

class TestCanFormMessage(unittest.TestCase):
    """Unit tests for the `can_form_message` function."""
    ...
    def test_unicode_and_symbols(self):
        ...
        self.assertEqual(result[1], "Missing characters: ñ(1), ú(1)")
    
    def test_numerical_characters(self):
        self.assertTrue(can_form_message("112", "112A")[0])
        result = can_form_message("1123", "112")
        self.assertFalse(result[0])
        self.assertEqual(result[1], "Missing characters: 3(1)")

    def test_empty_cases(self):
        self.assertEqual(can_form_message("", ""), (True, "Empty message can always be formed."))
        self.assertEqual(can_form_message("", "ANYTHING"), (True, "Empty message can always be formed."))
        self.assertEqual(can_form_message("A", ""), (False, "The chest is empty, cannot form any message."))
